...
import { connection, wallet_1, wsol } from "./constants";
...
export const getBuyTokenAmount = async (solAmount: bigint, poolId: PublicKey) => {
  // const pool_detail = await getPoolsWithPrices(mint);

  const pumpAmmSdk = new PumpAmmSdk(connection);
  const poolKey = new PublicKey(poolId);
  const user = wallet_1.publicKey;
  ...
  const isWsolBase = pool.baseMint.toBase58() === wsol
  // console.log(pool_detail)
  const sol_reserve = BigInt((isWsolBase ? poolBaseAmount : poolQuoteAmount).toString(10));
  const token_reserve = BigInt((isWsolBase ? poolQuoteAmount : poolBaseAmount).toString(10));
  ...
  return { amount_to_be_purchased, pool };
}
...
// async function main() {
//   const mint = new PublicKey("3nu6jFiztZJyjMfC2Lg7165jJzMGewJZwuuBVSpy8poV");
//   const pool_detail = await getPoolsWithPrices(mint);
//   console.log(pool_detail)
